...
//
// LogConsole.cpp: implementation of the LogConsole class.
//
//////////////////////////////////////////////////////////////////////
...
const int LogConsole::ToDebug   =  1;
const int LogConsole::ToFile    =  2;
const int LogConsole::ToConsole =  4;

const static int LINE_BUFFER_SIZE = 1024;

LogConsole::LogConsole(int mode, int level, LPTSTR filename, bool append)
{
    hlogfile = NULL;
    m_todebug = false;
    m_toconsole = false;
...
void LogConsole::SetMode(int mode) {
...
void LogConsole::SetLevel(int level) {
    m_level = level;
}

void LogConsole::SetFile(LPTSTR filename, bool append) 
{
    ...
    // If filename is NULL or invalid we should throw an ExceptionConsole here
    hlogfile = CreateFile(
        filename,  GENERIC_WRITE, FILE_SHARE_READ, NULL,
        OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL  );
    
    if (hlogfile == INVALID_HANDLE_VALUE) {
        // We should throw an ExceptionConsole here
        m_todebug = true;
        m_tofile = false;
...
#ifndef UNDER_CE
// Non-CE version 
void LogConsole::ReallyPrint(LPCTSTR format, va_list ap) 
{
...
void LogConsole::ReallyPrint(LPCTSTR format, va_list ap) 
{
    ...
    if (m_tofile && (hlogfile != NULL)) {
		...
		// LogConsole file is more readable if non-unicode!
